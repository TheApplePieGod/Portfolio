...
export const Navbar: React.FunctionComponent = (props) => {
    ...
    const isRouteSelected = React.useCallback(
        (route: string, exact: boolean) => {
            return exact
    ...
    const onScroll = React.useCallback(
        (e: WheelEvent) => {
            ...
            const currentRouteIndex = NAV_ROUTES.findIndex((e) =>
                isRouteSelected(e.route, e.exact)
            );
            if (currentRouteIndex != -1) {
                if (e.deltaY < 0 && currentRouteIndex > 0)
                    ...
                    e.deltaY > 0 &&
                    currentRouteIndex < NAV_ROUTES.length - 1
                )
                    router.push(NAV_ROUTES[currentRouteIndex + 1].route);
            }
            ...
            >
                {NAV_ROUTES.map((e, i) => {
                    ...
                    return (
                        ...
                        >
                            <HiddenA href={e.route} target="_self">
                                {e.name}
                            </HiddenA>
                        </NavCircle>
                    );
